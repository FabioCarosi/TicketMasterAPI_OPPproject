...
Stats stats = new StatsImplementation();
	...
	public JSONArray period(Vector<String> states, String subgenre, int period) throws WrongStateException, WrongValueException {
		...
		while(it.hasNext()) {	//ciclo che si conclude dopo aver scorso tutto il vettore con gli States
			...
			object = stats.getSubGenreEvents(it.next(), subgenre, period);	//chiamata al metodo getGenreEvents a cui si passa il genre i-esimo e il periodo preso in esame
		...
		subgenreEvents.put(maxStateEvents);	//Inserisco il JSONOBject dellos stati con più eventi all'interno del JSONArray finale
		subgenreEvents.put(minStateEvents);	//Inserisco il JSONOBject dellos stati con meno eventi all'interno del JSONArray finale
